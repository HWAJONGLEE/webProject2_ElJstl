...
public class BoardDao {
		...
		public ArrayList<Board> selectList(Connection conn, int currentPage, int limit) {
				...
				String query = "SELECT * " + 
						...
						"FROM (SELECT * FROM BOARD ORDER BY BOARD_REF DESC, " + 
						"BOARD_REPLY_REF DESC, BOARD_LEVEL ASC, " + 
						"BOARD_REPLY_SEQ ASC)) " + 
						"WHERE RNUM >=? AND RNUM <=?";
...
//				BOARD_READCOUNT
		...
		public int addReadCount(Connection conn, int boardNum) {
			...
			String query = "update board set board_readcount= board_readcount + 1 "
					+ "where board_num = ?";
		...
		public int updateReplySeq(Connection conn, Board reply) {
			...
			if(reply.getBoardLevel() == 1) {
					query = "update board set board_reply_seq = board_reply_seq + 1 "
